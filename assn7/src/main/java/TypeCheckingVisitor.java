...
public class TypeCheckingVisitor extends CalcLangBaseVisitor<Type> {
    ...
    private HashMap<String, Type> symbols = new HashMap<>(); // symbol table
    private HashMap<ParseTree, Type> nodeType = new HashMap<>();
    private HashMap<ParseTree, Type> coercion = new HashMap<>();

    private static HashMap<String, Type[]> functions; // built-in function types
    ...
    public Type visitIfExpr(CalcLangParser.IfExprContext ctx) {
        Type t0 = ctx.expr(0).accept(this);
        if(t0 != Type.BOOL) {
            return typeMismatch(ctx, t0, Type.BOOL);
        }
        Type t1 = ctx.expr(1).accept(this);
        Type t2 = ctx.expr(2).accept(this);
        Type resultType = unify(ctx.expr(1), t1, ctx.expr(2), t2);
        if(resultType == Type.ERROR) {
            return typeMismatch(ctx, t1, t2);
        }
        return trace(ctx, resultType);
    }
    ...
    public Type getCoercion(ParseTree ctx) {
